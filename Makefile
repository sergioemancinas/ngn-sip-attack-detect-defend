...
obs-smoke:
	...
	echo "==> Vector metrics"; for i in 1 2 3 4 5 6; do curl -sf http://127.0.0.1:9598/metrics >/dev/null 2>&1 && break || sleep 3; done; echo "vector_ metrics served: $$(curl -sf http://127.0.0.1:9598/metrics 2>/dev/null | grep -c '^vector_' || echo 0)"
