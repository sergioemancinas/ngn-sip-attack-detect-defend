...
| 11 | T-11 | Repudiation | P4 Suricata, F7 observation | Apple Silicon Docker capture limitations or missing PCAPs let an attacker dispute IDS evidence. | Medium | Medium | Accepted local-only risk: local MS work uses Suricata evidence for development, while production-like packet capture must be validated on Linux campus VM with saved PCAPs. | Suricata SIDs can fire, but absence of an alert is not proof of absence in local capture mode. |
