...
Walks one attack from script to dashboard, naming every component and the file where the wiring lives. Cross-references public docs/RFCs so each layer is traceable to its source.
...
| `attacks/01_recon/sippts_options_scan.sh` | Emits a label; real `sippts` packet path still stubbed (follow-up) | sippts: https://github.com/Pepelux/sippts |
