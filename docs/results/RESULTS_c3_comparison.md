...
| XGBoost | behavioural ML | ~1.00 | **0.00** | ~0.98 | **0.988 ± 0.015** |
...
XGBoost reaches F1 0.988 with a near-zero false-positive rate on the same data:
...
1. **DoS source attribution (open).** The SIPp-generated floods are not
   ...
   in `RESULTS_stage1_grouped.md`; flood detection needs destination-side rate
