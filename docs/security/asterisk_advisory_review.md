...
- Aikido MCP scan of `infra/asterisk/Dockerfile`, Asterisk config, and the Asterisk Compose service reports no issues after changing the Dockerfile to literal digest-pinned `FROM` lines and `USER asterisk`.
