...
- Aikido SAST/SCA/secrets/IaC continues to gate every PR via `.github/workflows/aikido.yml`; new code in `attacks/orchestrator/`, `siem/wazuh/`, and `infra/clickhouse/init/` is in scope for that gate.
...
- Re-evaluate Wazuh 4.10 -> 4.11/4.14 once Wazuh ships a CVE-mitigated image; coordinate with the Keycloak OIDC integration.
