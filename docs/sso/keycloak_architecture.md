...
| `ngn-sip-lab` | Lab operators, read-only reviewer accounts | `lab-admin` (role `wazuh-admin`, `all_access`) | All app clients listed below | OIDC SSO for every UI in the project. |
...
5. Add a read-only reviewer user with role mapping limited to dashboard `read` actions across all apps.
